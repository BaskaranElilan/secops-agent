"""
SecOps-Agent - Report Export Router (Phase 3)
...
class ExportRequest(BaseModel):
    ...
    title: str | None = "SecOps-Agent Security Report"
...
def _generate_pdf(messages: list[ExportMessage], title: str) -> bytes:
    ...
    elements.append(Paragraph("🛡️ SECOPS-AGENT", title_style))
    elements.append(Paragraph(f"{title}", styles['Heading2']))
    elements.append(Paragraph(
        f"Generated: {now} | Developed by Baskaran Elilan | {len(messages)} messages",
        subtitle_style
    ))
    ...
    for i, msg in enumerate(messages):
        if msg.role == 'user':
            ...
            elements.append(Paragraph(f"<b>🛡️ SECOPS-AGENT</b>", section_style))
    ...
    elements.append(Paragraph(
        f"SecOps-Agent v3.0 | Baskaran Elilan | Confidential",
        subtitle_style,
    ))
...
def _generate_simple_pdf(messages: list[ExportMessage], title: str) -> bytes:
    ...
    for msg in messages:
        prefix = "👤 USER" if msg.role == "user" else "🛡️ SECOPS-AGENT"
        lines.append(f"## {prefix}")
        lines.append(msg.content)
        lines.append("")
        lines.append("---")
        lines.append("")
    lines.append("SecOps-Agent v3.0 | Baskaran Elilan")
...
async def export_pdf(req: ExportRequest):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="secops-agent-report-{datetime.datetime.now().strftime("%Y%m%d-%H%M")}.pdf"'},
    )
...
async def export_markdown(req: ExportRequest):
    ...
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = [
        f"# 🛡️ SecOps-Agent - {req.title or 'Security Report'}",
        f"> Generated: {now} | Developed by Baskaran Elilan",
        "",
        "---",
        "",
    ]
    for msg in req.messages:
        prefix = "👤 You" if msg.role == "user" else "🛡️ SecOps-Agent"
        lines.append(f"## {prefix}")
        lines.append("")
        lines.append(msg.content)
        lines.append("")
        lines.append("---")
        lines.append("")
    lines.append(f"*SecOps-Agent v3.0 | Baskaran Elilan*")

    md_bytes = "\n".join(lines).encode("utf-8")
    return StreamingResponse(
        io.BytesIO(md_bytes),
        media_type="text/markdown",
        headers={"Content-Disposition": f'attachment; filename="secops-agent-report-{datetime.datetime.now().strftime("%Y%m%d-%H%M")}.md"'},
    )
