...
SEED_PLAN = [
    ("winlogbeat-secops-agent", gen_failed_login, 80),
    ("winlogbeat-secops-agent", gen_success_login, 100),
    ("winlogbeat-secops-agent", gen_explicit_creds, 25),
    ("winlogbeat-secops-agent", gen_process, 60),
    ("winlogbeat-secops-agent", gen_powershell, 40),
    ("packetbeat-secops-agent", gen_dns, 80),
    ("filebeat-secops-agent", gen_linux_auth, 50),
    ("filebeat-secops-agent", gen_firewall, 40),
    ("security-alerts-secops-agent", gen_alert, 30),
]
...
async def seed_elk_logs():
    ...
    try:
        async with httpx.AsyncClient(timeout=10, verify=False) as c:
            r = await c.get(f"{base_url}/winlogbeat-secops-agent/_count")
