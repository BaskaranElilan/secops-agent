"""
SecOps-Agent - Security Middleware
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    If SECOPS_AGENT_API_KEY is set in .env, all non-public endpoints
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not settings.secops_agent_api_key:
            return await call_next(request)
        ...
        if not api_key or api_key != settings.secops_agent_api_key:
            return JSONResponse(
                status_code=401,
                content={
                    "error": "Unauthorized",
