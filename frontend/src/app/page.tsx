...
export default function Dashboard() {
  ...
  const exportMd = () => {
    const md = msgs.map(m => `## ${m.role === 'user' ? '👤 You' : '🛡️ SecOps-Agent'}\n\n${m.content}`).join('\n\n---\n\n')
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([`# SecOps-Agent Export\n\n${md}`], { type: 'text/markdown' })); a.download = `secops-agent-${Date.now()}.md`; a.click()
  }

  const exportPdf = async () => {
    try { const blob = await exportPDF(msgs.map(m => ({ role: m.role, content: m.content }))); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `secops-agent-report-${Date.now()}.pdf`; a.click() } catch { exportMd() }
  }
      ...
      <aside className="hidden xl:block bg-gradient-to-b from-cs-bg2 to-[#0a0e1a] border-l border-cs-border overflow-y-auto p-4">
        ...
        <div>
          ...
          <div className="bg-gradient-to-br from-cs-orange/10 to-cs-purple/10 border border-cs-orange/20 rounded-lg p-3 text-center">
            <span className="font-bold text-cs-orange text-sm">Baskaran Elilan</span>
            <div className="text-[0.5rem] text-cs-text3 font-mono mt-1">SecOps-Agent v3.0 | Agentic</div>
          </div>
        </div>
      </aside>
